...
public class TofuEncasedCogVisual extends KineticBlockEntityVisual<KineticBlockEntity> {
    ...
    public static TofuEncasedCogVisual small(VisualizationContext modelManager, KineticBlockEntity blockEntity, float partialTick) {
        return new TofuEncasedCogVisual(modelManager, blockEntity, false, partialTick, Models.partial(ModAllPartialModels.SHAFTLESS_COGWHEEL));
    }

    public static TofuEncasedCogVisual large(VisualizationContext modelManager, KineticBlockEntity blockEntity, float partialTick) {
        return new TofuEncasedCogVisual(modelManager, blockEntity, true, partialTick, Models.partial(ModAllPartialModels.SHAFTLESS_LARGE_COGWHEEL));
    }

    public TofuEncasedCogVisual(VisualizationContext modelManager, KineticBlockEntity blockEntity, boolean large, float partialTick, Model model) {
        ...
        if (block instanceof IRotate def) {
            for (Direction d : Iterate.directionsInAxis(rotationAxis())) {
                if (!def.hasShaftTowards(blockEntity.getLevel(), blockEntity.getBlockPos(), blockState, d))
                    continue;
                RotatingInstance instance = instancerProvider().instancer(AllInstanceTypes.ROTATING, Models.partial(ModAllPartialModels.SHAFT_HALF))
                        .createInstance();
