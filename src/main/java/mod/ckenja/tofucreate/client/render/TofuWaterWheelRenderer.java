package mod.ckenja.tofucreate.client.render;
...
public class TofuWaterWheelRenderer<T extends WaterWheelBlockEntity> extends KineticBlockEntityRenderer<T> {
    public static final SuperByteBufferCache.Compartment<TofuWaterWheelRenderer.ModelKey> WATER_WHEEL = new SuperByteBufferCache.Compartment<>();
    ...
    public TofuWaterWheelRenderer(BlockEntityRendererProvider.Context context, boolean large) {
        super(context);
        this.large = large;
    }

    public static <T extends WaterWheelBlockEntity> TofuWaterWheelRenderer<T> standard(BlockEntityRendererProvider.Context context) {
        return new TofuWaterWheelRenderer<>(context, false);
    }

    public static <T extends WaterWheelBlockEntity> TofuWaterWheelRenderer<T> large(BlockEntityRendererProvider.Context context) {
        return new TofuWaterWheelRenderer<>(context, true);
    }

    @Override
    protected SuperByteBuffer getRotatedModel(T be, BlockState state) {
        TofuWaterWheelRenderer.ModelKey key = new TofuWaterWheelRenderer.ModelKey(large, state, be.material);
        return SuperByteBufferCache.getInstance().get(WATER_WHEEL, key, () -> {
            BakedModel model = generateModel(key);
            BlockState state1 = key.state();
            Direction dir;
            if (key.large()) {
                dir = Direction.fromAxisAndDirection(state1.getValue(LargeWaterWheelBlock.AXIS), Direction.AxisDirection.POSITIVE);
            } else {
    ...
    public static BakedModel generateModel(TofuWaterWheelRenderer.ModelKey key) {
        return generateModel(TofuWaterWheelRenderer.Variant.of(key.large(), key.state()), key.material());
    }

    public static BakedModel generateModel(TofuWaterWheelRenderer.Variant variant, BlockState material) {
    ...
    public enum Variant {
        SMALL(ModAllPartialModels.WATER_WHEEL),
        LARGE(ModAllPartialModels.LARGE_WATER_WHEEL),
        LARGE_EXTENSION(ModAllPartialModels.LARGE_WATER_WHEEL_EXTENSION),
        ;
        ...
        public static TofuWaterWheelRenderer.Variant of(boolean large, BlockState blockState) {
            if (large) {
